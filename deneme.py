...
class MyModel(nn.Module):
	def __init__(self, my_pretrained_model):
		...
		self.firstlayer = nn.Conv2d(1, 3, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))
		self.pretrained = my_pretrained_model
		self.my_new_layers = nn.Sequential(
...
######################### Testing Models #################################
	...
	model.classifier.add_module(name="Ekmodül",module=module)
	test=True

elif TopModelName=="resnet18_test":
	module=nn.modules
	module = nn.Sequential(
	    nn.Linear(in_features=1000, out_features=256, bias=True),
	    nn.ReLU(inplace=True),
	...
	model.fc.add_module(name="Ekmodül",module=module)
	test=True

elif TopModelName=="resnet18_test2":
	model = MyModel(my_pretrained_model=models.resnet18(pretrained=True, progress=True))
	test=True
